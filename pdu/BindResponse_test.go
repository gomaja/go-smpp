...
func TestBindResponseRoundTrip(t *testing.T) {
	...
	t.Run("zeroPaddedFailedTransmitterAndReceiver", func(t *testing.T) {
		// SMPP v3.4 Issue 1.2 Sections 4.1.2 and 4.1.4 state that failed
		// bind_transmitter_resp and bind_receiver_resp PDUs omit the body.
		// Some SMSC simulators send the absent body as NUL padding; that must not
		// be parsed as malformed optional TLVs.
		for _, raw := range []string{
			"00000020800000020000000f0000000400000000000000000000000000000000",
			"00000020800000010000000f0000000400000000000000000000000000000000",
		} {
			p, err := Parse(bytes.NewReader(fromHex(raw)))
			require.NoError(t, err)

			resp, ok := p.(*BindResp)
			require.True(t, ok)
			require.Equal(t, data.ESME_RINVSYSID, resp.CommandStatus)
			require.Empty(t, resp.SystemID)
			require.Empty(t, resp.OptionalParameters)
		}
	})
