...
import (
	...
	"github.com/gomaja/go-smpp/errors"
)
...
func (c *ShortMessage) SetMessageWithEncoding(message string, enc data.Encoding) (err error) {
	if c.messageData, err = enc.Encode(message); err == nil {
		...
		if c.enc == data.GSM7BITPACKED { // to prevent unwanted "@"
			...
			if (nSeptet+1)%8 == 0 {
				c.messageData[len(c.messageData)-1] = (c.messageData[len(c.messageData)-1] & 0x01) | (0x0D << 1)
			}
		}
	}
	return
...
// split splits one short message into multiple short messages with UDH according
// to 3GPP TS 23.040 section 9.2.3.24.1.
//
...
func (c *ShortMessage) split() (multiSM []*ShortMessage, err error) {
	...
	// Reserve 6 octets for the concatenated-message UDH defined by 3GPP
	// TS 23.040 section 9.2.3.24.1.
