...
// Tag is the tag of a Tag-Length-Value (TLV) optional parameter; see SMPP v3.4
// Issue 1.2 sections 3.2.4.1 and 5.3.2. Portions of these definitions are
// derived from fiorix/go-smpp; see LICENSES/fiorix-go-smpp-MIT.txt.
type Tag uint16
...
// NewField returns a TLV field with a defensive copy of data.
func NewField(tag Tag, data []byte) Field {
	return Field{
		Tag:  tag,
...
// NewFieldUint8 returns an Integer TLV field with a 1-octet value.
// SMPP v3.4 Issue 1.2 section 3.1 defines Integer fields as unsigned and
// transmitted MSB-first.
func NewFieldUint8(tag Tag, value uint8) Field {
	return Field{Tag: tag, Data: []byte{value}}
}

// NewFieldUint16 returns an Integer TLV field with a 2-octet value.
// SMPP v3.4 Issue 1.2 section 3.1 defines Integer fields as unsigned and
// transmitted MSB-first.
func NewFieldUint16(tag Tag, value uint16) Field {
	var data [2]byte
	binary.BigEndian.PutUint16(data[:], value)
	return NewField(tag, data[:])
}

// NewFieldUint32 returns an Integer TLV field with a 4-octet value.
// SMPP v3.4 Issue 1.2 section 3.1 defines Integer fields as unsigned and
// transmitted MSB-first.
func NewFieldUint32(tag Tag, value uint32) Field {
	var data [4]byte
...
// Uint8 returns the TLV value as an exact-width 1-octet Integer.
func (t Field) Uint8() (uint8, error) {
	if len(t.Data) != 1 {
		return 0, fmt.Errorf("tlv %s has %d octets, want 1", t.Tag.Hex(), len(t.Data))
	}
	return t.Data[0], nil
}

// Uint16 returns the TLV value as an exact-width 2-octet Integer.
func (t Field) Uint16() (uint16, error) {
	if len(t.Data) != 2 {
		return 0, fmt.Errorf("tlv %s has %d octets, want 2", t.Tag.Hex(), len(t.Data))
	}
	return binary.BigEndian.Uint16(t.Data), nil
}

// Uint32 returns the TLV value as an exact-width 4-octet Integer.
func (t Field) Uint32() (uint32, error) {
	if len(t.Data) != 4 {
		return 0, fmt.Errorf("tlv %s has %d octets, want 4", t.Tag.Hex(), len(t.Data))
	}
