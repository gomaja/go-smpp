...
func TestReceive(t *testing.T) {
	...
	require.Equal(t, testSMSCSystemID, receiver.Receiver().SystemID())
...
func TestReceivableHandleOrCloseReportsAutoRespondedPDU(t *testing.T) {
	var (
		gotPDU       pdu.PDU
		gotResponded bool
		gotResponse  pdu.PDU
	...
	r := &receivable{
		settings: Settings{
			...
			OnPDU: func(p pdu.PDU, responded bool) {
				gotPDU = p
				gotResponded = responded
			},
		},
	}

	require.False(t, r.handleOrClose(pdu.NewEnquireLink()))
	require.IsType(t, &pdu.EnquireLink{}, gotPDU)
	require.True(t, gotResponded)
	require.IsType(t, &pdu.EnquireLinkResp{}, gotResponse)
}
